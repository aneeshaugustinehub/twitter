...
export default function Messages() {
  ...
  return (
    <div className="flex w-full max-w-[1000px] min-h-screen border-x border-gray-800">
          ...
          {filteredConvos.length === 0 ? (
            ...
            filteredConvos.map((c, i) => (
              <div
                ...
                className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors hover:bg-gray-900 border-b border-gray-800/50 ${activeId === c.id ? "bg-gray-900" : ""}`}
              >
                <div className="relative flex-shrink-0">
                  <div className={`w-12 h-12 rounded-full ${avatarColors[i % avatarColors.length]} flex items-center justify-center text-white font-semibold text-sm`}>
                    {c.init}
                  </div>
                  {c.online && (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <p className={`text-sm truncate ${c.unread > 0 ? "text-white font-medium" : "text-gray-500"}`}>
                      {c.lastMsg}
                    </p>
                    {c.unread > 0 && (
      ...
      {activeId ? (
        <div className="flex flex-col flex-1 min-w-0">
          ...
          {/* Chat header */}
          <div className="sticky top-0 z-10 bg-black/80 backdrop-blur-sm px-4 py-3 border-b border-gray-800 flex items-center gap-3">
            <button
            ...
            <div className="relative flex-shrink-0">
              <div className={`w-10 h-10 rounded-full ${avatarColors[convos.findIndex(c=>c.id===activeId) % avatarColors.length]} flex items-center justify-center text-white font-semibold text-sm`}>
                {activeConvo?.init}
              </div>
              {activeConvo?.online && (
          ...
          <div className="border-t border-gray-800 px-4 py-3">
            <div className="flex items-center gap-2 bg-gray-900 rounded-2xl px-3 py-2">
        ...
        <div className="hidden md:flex flex-col flex-1 items-center justify-center text-center px-8">
          <p className="text-3xl font-bold text-white mb-2">Select a message</p>
          <p className="text-gray-500 text-sm max-w-xs">Choose from your existing conversations, or start a new one.</p>
          <button className="mt-6 bg-sky-500 hover:bg-sky-400 text-white font-semibold px-6 py-2.5 rounded-full transition-colors text-sm">
            New message
          </button>
        </div>
      )}
